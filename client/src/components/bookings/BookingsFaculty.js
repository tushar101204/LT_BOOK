...
const BookingFaculty = () => {
  ...
  const fetchBookings = async () => {
    try {
      ...
      setBookingData(response.data.bookings); // Update bookings state with the fetched data
    } catch (error) {
                    ...
                    filteredBookings.map((booking) => (
                      ...
                      {booking.eventDateType === "multiple" ? 
                        ...
                        <td className="px-5 py-5 text-m bg-white  border-gray-200">
                          ...
                          <button
                            onClick={() => deleteBooking(booking._id)} className="text-m font-bold ml-5 leading-none text-gray-600 py-3 px-5 bg-red-200 rounded hover:bg-red-300 focus:outline-none">Cancel</button>
                        </td>
